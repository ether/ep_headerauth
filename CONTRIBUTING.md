# Contributing to ep_headerauth

Thanks for helping improve `ep_headerauth` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_headerauth` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_headerauth from your local clone
pnpm run plugins i --path ../ep_headerauth

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_headerauth/` are picked up after a server restart.
...
* Etherpad version, ep_headerauth version, browser + OS
* Steps to reproduce
