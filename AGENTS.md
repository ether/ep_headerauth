...
`ep_headerauth` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_headerauth`:
...
* Server: `authenticate`, `handleMessage`, `loadSettings`
